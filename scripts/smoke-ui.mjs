...
const checks = [
  ...
  ["run header remains minimal with copy-link hotkey", "Hotkeys: ?/H help · E interact · M menu · R new run · V copy link"],
  ...
  ["essence shop pity ready label", "• Shop Pity Ready"],
  ...
  ["skills section compact header", "<I src={bootsIcon}/>Skills"],
  ...
  ["control pad toggle state", "const [showControlPad,setShowControlPad] = useState(false)"],
  ["utility panel toggle state", "const [showUtilityPanel,setShowUtilityPanel] = useState(false)"],
  ["build economy toggle copy", "{showBuildPanel ? 'Hide' : 'Show'}"],
  ["control pad toggle copy", "{showControlPad ? 'Hide Pad' : 'Controls'}"],
  ["utility panel toggle copy", "{showUtilityPanel ? 'Hide Tools' : 'Tools'}"],
  ["icon legend toggle state", "const [showIconLegend,setShowIconLegend] = useState(false)"],
  ["icon legend toggle label", "{showIconLegend ? 'Hide Icons' : 'Icon Legend'}"],
  ["icon legend panel class", "className='dq-item dq-icon-legend'"],
  ...
  ["action log immersive (no tick stamp)", "void tick"],
  ...
  ["action log dash used flavor", "You burst forward in a quick dash."],
  ["action log merchant close flavor", "You step away; the Merchant closes shop."],
  ["action log spirit pickup flavor", "You recover a *${p?.core?.spirit || 'spirit'}* core."],
  ["action log spirit reroll telemetry flavor", "The weave resists repetition: spirit drop rerolled from *${p?.from || 'prior'}* to *${p?.to || 'new'}*."],
  ["action log gear equipped flavor", "You ready *${p?.name || 'gear'}*."],
  ["action log chest opened flavor", "A chest cracks open: *${itemLabel(p?.drop)}*."],
  ["action log rift used flavor", "The rift howls, dragging ${p?.pulled ?? 0} foes inward."],
  ["action log stairs used flavor", "You descend to floor ${p?.toFloor ?? '?'}."],
  ...
  ["hotbar helper text removed for lean HUD", "{/* intentionally no helper copy here to keep HUD minimal */}"],
...
const mountChecks = [
  ...
  ["mount entity depth helper", "const entityDepth = (ent:any)=>{"],
  ["mount cleanup helper for stale displays", "const pruneMissingEntityDisplays = (state:any)=>{"],
  ...
  ["mount enforces even tile size", "const tileSize = rawTile % 2 === 0 ? rawTile : Math.max(8, rawTile - 1)"],
  ...
  ["walls render without heuristic rotation", "wall.setDisplaySize(tileSize, tileSize)"],
  ...
  ["mount sets sprite depth by entity type", "s.setDepth(entityDepth(ent))"],
  ["mount prunes stale item sprites on pickup", "if(st) pruneMissingEntityDisplays(st)"],
]
...
const engineChecks = [
  ...
  ["engine spirit pity trigger", "const pityReady = this.spiritDryFloors >= this.shopSpiritPityThreshold"],
  ["engine spirit anti-repeat telemetry", "this.emit({tick:this.tick,type:'spirit_drop_rerolled',payload:{from:previousSpirit,to:core.spirit,floor:this.floor}})"],
