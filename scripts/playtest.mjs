...
function shortestFirstStep(state, start, goal){
  ...
  while(q.length){
    ...
    for(const d of dirs){
      ...
      seen.add(nk)
      prev.set(nk, {from:key(cur), dir:d.dir})
      q.push(n)
    }
  }
...
function chooseAction(state){
  ...
  const relic = state.entities.find(e=>e.type==='item' && e.kind==='relic')

  let targets = []
  if(stairs) targets = [stairs.pos]
  else if((p.hp||0)<=5 && potion) targets = [potion.pos]
  else if(monsters.length){
    monsters.sort((a,b)=>manhattan(p.pos,a.pos)-manhattan(p.pos,b.pos))
    targets = monsters.slice(0,2).map(m=>m.pos)
  } else if(relic) targets = [relic.pos]
...
const runs = []
for(let i=0;i<8;i++){
  const eng = new Engine(30,30, i+1)
  let state = eng.getState()
  let turns = 0
  while(!state.gameOver && turns < 1400){
    state = eng.step(chooseAction(state))
    turns++
  }
  const hp = state.entities.find(e=>e.id==='p')?.hp ?? 0
  runs.push({seed:i+1,floor:state.floor,score:state.score,hp,turns,defeat:state.gameOver})
}

const avg = (k)=> runs.reduce((a,b)=>a+b[k],0)/runs.length
const summary = {
  runs,
  avgFloor: Number(avg('floor').toFixed(2)),
  avgScore: Math.round(avg('score')),
  avgTurns: Math.round(avg('turns')),
  defeats: runs.filter(r=>r.defeat).length,
  maxFloor: Math.max(...runs.map(r=>r.floor))
}
